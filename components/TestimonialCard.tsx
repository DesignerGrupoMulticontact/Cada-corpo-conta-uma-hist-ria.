...
export const TestimonialCard: React.FC<TestimonialCardProps> = ({ text, onClose, icon: Icon, author, location, tag, date }) => {
  ...
  useEffect(() => {
    // Generate a modest random number between 12 and 86 for social proof simulation
  ...
  const getShareUrl = () => {
    ...
    // Prevent sharing internal blob/localhost URLs
  ...
  const handleSocialShare = (platform: 'whatsapp' | 'facebook' | 'twitter' | 'linkedin') => {
    ...
    switch (platform) {
      case 'whatsapp':
        ...
        break;
      case 'facebook':
        // Facebook only allows URL sharing via sharer.php
  ...
  return (
    <motion.div
      ...
      // Use relative positioning so it sits nicely in the flex center container of PortugalMap
      // Adjusted max-h for mobile split view safety (safe up to ~60vh)
    ...
    >
      <div className="relative flex flex-col overflow-visible rounded-2xl bg-white/95 backdrop-blur-xl border border-white/60 shadow-2xl h-full">
        ...
        <div className="p-5 md:p-6 overflow-y-auto custom-scrollbar flex-1">
            ...
            {/* Footer: Social Proof / Interaction */}
            ...
            <div className="flex items-center gap-2">
                ...
                {/* Particle effect simulation for reaction */}
                <AnimatePresence>
                    {hasReacted && (
                    <motion.span
