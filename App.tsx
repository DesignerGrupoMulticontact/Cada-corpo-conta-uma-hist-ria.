...
const Counter = ({ value }: { value: number }) => {
  // Slower, smoother spring for better visual of the counting up
...
const App: React.FC = () => {
  ...
  // State for Map Focus Control
  const [activeStoryId, setActiveStoryId] = useState<string | null>(null);
  
  // State for User Submission Toast Logic
  ...
  // Base count to match the requested ~2590 range + dynamic additions
  ...
  // Transform values for parallax layers (Desktop Only)
  ...
  const handleAddStory = (newStory: Testimonial) => {
    ...
    // Store it temporarily. We will trigger the toast only when the modal closes.
    setPendingUserStory(newStory);
  };

  const handleModalClose = () => {
    setIsModalOpen(false);
    // If there was a pending story submitted, trigger the specific toast now
    if (pendingUserStory) {
        setToastOverrideStory(pendingUserStory);
        // Clear pending so it doesn't trigger again
